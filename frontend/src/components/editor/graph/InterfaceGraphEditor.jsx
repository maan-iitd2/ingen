...
// nodeTypes must be stable (defined outside the component) to avoid ReactFlow re-rendering all nodes.
const nodeTypes = {
  source:      SourceNode,
...
function GraphEditorInner({ interfaceName, iface }) {
  ...
  }, [iface, model, setNodes, setEdges, fitView]);
  ...
  }, [apply, iface, feedConnectionSpec, chainConnectionSpec]);
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if ((e.ctrlKey || e.metaKey) && (e.key === 'Z' || (e.shiftKey && e.key === 'z'))) { e.preventDefault(); redo(); }
      if (e.key === 'f' || e.key === 'F') fitView({ padding: 0.2, duration: 300 });
      if (e.key === 'Escape') { setSelectedNodeId(null); closeCtx(); }
    };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
  }, [undo, redo, fitView, setSelectedNodeId, closeCtx]);
