...
export default function ListControls({ index, count, onMove, onRemove }) {
  return (
    <div className="lctrls">
      <button className="lctrls__btn" title="Move up" disabled={index === 0} onClick={() => onMove(-1)}>↑</button>
      <button className="lctrls__btn" title="Move down" disabled={index === count - 1} onClick={() => onMove(1)}>↓</button>
      <button className="lctrls__btn lctrls__btn--del" title="Remove" onClick={onRemove}>✕</button>
    </div>
  );
}
