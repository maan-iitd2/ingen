...
//  paragraphs separated by blank lines. Intentionally minimal — no HTML injection.
...
function renderInline(text) {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    if (token.startsWith('`') && token.endsWith('`')) {
      ...
      // link
      parts.push(<a key={m.index} href={m[3]} target="_blank" rel="noopener noreferrer" className="mm-link">{m[2]}</a>);
    }
